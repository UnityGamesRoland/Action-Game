...
{
	public string textureSetName = "New Texture Set";
	public int targetFootstepTypeIndex = 0;
	public int[] acceptedTerrainIndexes;
}
...
public class PlayerMotorBehavior : MonoBehaviour
{
	...
	private void Start()
	{
		...
		focus = FindObjectOfType<FocusTime>();
	...
	private void CalculateMovement()
	{
		...
		if(focus != null && focus.isFocusing)
		{
	...
	private void OnControllerColliderHit(ControllerColliderHit hit)
	{
		...
		{
			int surfaceIndex = GetMainTexture(transform.position);

			for(int a = 0; a < terrainTextures.Length; a++)
			{
				for(int b = 0; b < terrainTextures[a].acceptedTerrainIndexes.Length; b++)
				{
					if(terrainTextures[a].acceptedTerrainIndexes[b] == surfaceIndex)
					{
						int targetFootstep = terrainTextures[a].targetFootstepTypeIndex;

						currentStepSound = footstepTypes[targetFootstep].sounds[Random.Range(0, footstepTypes[targetFootstep].sounds.Length)];
						currentStepVolume = footstepTypes[targetFootstep].volume;
						break;
					}
				}
			}
		...
		else
		{
			...
			for(int i = 0; i < footstepTypes.Length; i++)
			{
				...
				if(hit.transform.tag == footstepTypes[i].tagName)
				{
