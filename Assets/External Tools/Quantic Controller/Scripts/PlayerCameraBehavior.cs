...
public class PlayerCameraBehavior : MonoBehaviour
{
	...
	private Vector3 recoilAmount;
	private float horizontalVelocity;
	private float verticalVelocity;
	private bool increaseRecoil;
	...
	private void Update()
	{
		...
		if(increaseRecoil)
		{
			currentRecoilX = Mathf.SmoothDamp(currentRecoilX, recoilAmount.x, ref horizontalVelocity, recoilAmount.z/2);
			currentRecoilY = Mathf.SmoothDamp(currentRecoilY, recoilAmount.y, ref verticalVelocity, recoilAmount.z/2);
		}

		else
		{
			if(currentRecoilX != 0) currentRecoilX = Mathf.SmoothDamp(currentRecoilX, 0, ref horizontalVelocity, recoilAmount.z/2);
			if(currentRecoilY != 0) currentRecoilY = Mathf.SmoothDamp(currentRecoilY, 0, ref verticalVelocity, recoilAmount.z/2);
		}
	}
	...
	private void HandleMouseLook()
	{
		...
		currentRotation = Vector3.SmoothDamp(currentRotation, new Vector3(pitch + currentRecoilX, yaw + currentRecoilY), ref rotationSmoothVelocity, mouseSmoothScale);
	...
	public void RecoilCamera(float horizontalRecoil, float verticalRecoil, float recoilSpeed)
	{
		recoilAmount = new Vector3(verticalRecoil, horizontalRecoil, recoilSpeed);
		StopAllCoroutines();
		StartCoroutine(TimeRecoil());
	}

	private IEnumerator TimeRecoil()
	{
		increaseRecoil = true;
		yield return new WaitForSeconds(recoilAmount.z);
